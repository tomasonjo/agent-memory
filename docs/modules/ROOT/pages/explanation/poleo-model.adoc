...
image::diagrams/poleo-model.png[POLE+O entity model: Person, Object, Location, Event, and Organization node types with their subtypes,width=600,align=left]
...
async with MemoryClient(settings) as memory:
    ...
    # Get entity by name
    entity = await memory.long_term.get_entity_by_name(
        name="Acme Corp",
