...
image::diagrams/embedding-based-similarity.png[Embedding-based entity resolution: new entity → embedding → vector similarity search → similarity scores → merge/flag/create,width=540,align=left]
...
duplicates = await memory.long_term.find_potential_duplicates(limit=50)
...
for entity1, entity2, confidence in duplicates:
    ...
    if user_confirms_same_entity():
        await memory.long_term.review_duplicate(entity1.id, entity2.id, confirm=True)
    else:
        await memory.long_term.review_duplicate(entity1.id, entity2.id, confirm=False)
----
...
for product1, product2, confidence in duplicates:
    ...
    print(f"  SKU 1: {product1.metadata.get('sku')}")
    print(f"  SKU 2: {product2.metadata.get('sku')}")

    # If same SKU, definitely merge
    if product1.metadata.get('sku') == product2.metadata.get('sku'):
        await memory.long_term.review_duplicate(product1.id, product2.id, confirm=True)
----
...
entity, result = await memory.long_term.add_entity(
...
[source,python]
----
stats = await memory.long_term.get_deduplication_stats()
...
daily_review = await memory.long_term.find_potential_duplicates(limit=50)

# Weekly review of lower-confidence flags
weekly_review = await memory.long_term.find_potential_duplicates(limit=200)
----
