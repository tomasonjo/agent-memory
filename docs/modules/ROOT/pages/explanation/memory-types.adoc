...
image::diagrams/the-three-layer-memory-architecture.png[Three-layer memory architecture: short-term, long-term, and reasoning memory backed by Neo4j,width=600,align=left]

image::diagrams/three-memory-types-diagram.png[Detailed breakdown of all three memory types: short-term (Conversation/Message chain), long-term (POLE+O entities, Preference, Fact), and reasoning (Trace, Step, ToolCall) with their key capabilities,width=600,align=left]
