...
image::diagrams/reasoning-trace-graph.png[Reasoning trace graph: Message triggers a ReasoningTrace with Steps, each Step using ToolCalls that TOUCH entities,width=540,align=left]
...
step1 = await client.reasoning.add_step(
    trace_id=trace.id,
    thought="Searching product catalog for running shoes",
    action="Customer asked for running shoes, starting with catalog search",
)

# Step 2: Get preferences
step2 = await client.reasoning.add_step(
    trace_id=trace.id,
    thought="Retrieved customer preferences for brand and style",
    action="Need to personalize results based on stored preferences",
)

# Step 3: Filter and rank
step3 = await client.reasoning.add_step(
    trace_id=trace.id,
    thought="Filtered products by Nike brand preference, ranked by rating",
    action="Customer prefers Nike, so boosting those results",
)
----
...
await client.reasoning.complete_trace(
    trace_id=trace.id,
    outcome="Recommended 3 Nike running shoes matching customer preferences",
    success=True,
)
...
await client.reasoning.complete_trace(
    trace_id=trace.id,
    outcome="No products found matching criteria - inventory empty for running_shoes",
    success=False,
)
----
...
async def run_with_tracing(user_query: str, user_id: str):
    ...
    try:
        ...
        await memory_client.reasoning.complete_trace(
            trace_id=trace.id,
            outcome=f"Successfully processed: {result.data}",
            success=True,
        )
...
class ReasoningTraceCallback(BaseCallbackHandler):
    ...
    async def on_chain_end(self, outputs, **kwargs):
        ...
        await self.memory_client.reasoning.complete_trace(
            trace_id=self.trace_id,
            outcome="Chain completed successfully",
            success=True,
        )
...
await client.reasoning.add_step(
    trace_id=trace.id,
    thought="Filtered 20 products down to 5 matching Nike brand preference",
    observation="Customer has strong Nike preference (confidence: 0.95). Filtering to boost relevance.",
    metadata={
        "input_count": 20,
        "output_count": 5,
...
[source,python]
----
trace = await client.reasoning.start_trace(session_id=session_id, task=task)

try:
    result = await execute_task()
    await client.reasoning.complete_trace(
        trace_id=trace.id,
        outcome=f"Task completed: {result}",
        success=True,
    )
except Exception as e:
    await client.reasoning.complete_trace(
...
await client.reasoning.complete_trace(
    trace_id=trace.id,
    outcome="Partial success: completed product_search and preference_matching, skipped inventory_check due to inventory_service_unavailable",
    success=True,
)
----
