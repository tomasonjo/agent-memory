...
async def import_product_catalog(products: list[dict]):
    ...
    for i, product in enumerate(products):
        ...
        product_entity, _ = await client.long_term.add_entity(
            name=product["name"],
            entity_type="PRODUCT",
            description=product.get("description"),
            attributes={
        ...
        if product.get("brand"):
            brand_entity, _ = await client.long_term.add_entity(
                name=product["brand"],
                entity_type="BRAND",
            )
        ...
        if product.get("category"):
            category_entity, _ = await client.long_term.add_entity(
        ...
        if result.items[i].success:
            for entity in result.items[i].entities:
                if entity.type not in ["PRODUCT", "BRAND"]:  # Avoid duplicates
                    linked, _ = await client.long_term.add_entity(
                        name=entity.name,
                        entity_type=entity.type,
                    )
