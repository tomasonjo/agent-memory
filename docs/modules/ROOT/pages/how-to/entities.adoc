...
client_entity, _ = await client.long_term.add_entity(
...
security, _ = await client.long_term.add_entity(
    name="Apple Inc.",
...
image::diagrams/background-entity-enrichment.png[Background enrichment flow: entity created → enrichment queue → Wikipedia/Diffbot → entity updated with description and metadata,width=540,align=left]
...
client = MemoryClient(settings, enrichment_provider=WikimediaEnricher())
...
brand, _ = await client.long_term.add_entity(
    name="Nike",
    entity_type="BRAND",
    attributes={"headquarters": "Oregon"}
)

product, _ = await client.long_term.add_entity(
    name="Nike Air Max 90",
    entity_type="PRODUCT",
    attributes={"brand": "Nike", "category": "Running"}
)
