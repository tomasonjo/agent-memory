...
image::diagrams/preference-aware-context-graph.png[Preference-aware context graph: Customer node connected via HAS_PREFERENCE relationships to Brand, Shipping, and Style preference nodes,width=600,align=left]
...
brand_prefs = await client.long_term.search_preferences(
    query="brand",
)

# Get investment-related preferences
investment_prefs = await client.long_term.search_preferences(
    query="investment risk time horizon",
)
----
...
async def get_recommendation_context(user_id: str, product_category: str) -> dict:
    ...
    preferences = await client.long_term.search_preferences(
        query="brand style size price range",
    )
...
async def get_investment_context(client_id: str) -> dict:
    ...
    investment_prefs = await client.long_term.search_preferences(
        query="risk profile investment style time horizon exclusions",
    )

    # Tax-related preferences
    tax_prefs = await client.long_term.search_preferences(
        query="tax strategy",
    )

    # Communication preferences
    comm_prefs = await client.long_term.search_preferences(
        query="communication",
    )
...
async def decay_stale_preferences(user_id: str, decay_after_days: int = 90):
    ...
    preferences = await client.long_term.search_preferences(query="")
...
async def infer_preferences_from_history(user_id: str):
    ...
    purchases = await client.graph.execute_read(
        """
...
async def get_preference_filtered_products(
    ...
    style_prefs = await client.long_term.search_preferences(
        query="style",
    )
