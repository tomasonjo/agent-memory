...
# Batching and streaming are configured at call time on
# ``ExtractionPipeline.extract_batch()`` / ``StreamingExtractor``.
# GLiREL relation extraction is opt-in via ``GLiRELExtractor``.
----
...
[source,bash]
----
NAM_SCHEMA_CONFIG__MODEL=poleo                   # poleo, legacy, custom
NAM_SCHEMA_CONFIG__ENTITY_TYPES='["PERSON","PRODUCT"]'  # For custom model
NAM_SCHEMA_CONFIG__ENABLE_SUBTYPES=true
NAM_SCHEMA_CONFIG__STRICT_TYPES=false
NAM_SCHEMA_CONFIG__CUSTOM_SCHEMA_PATH=/path/to/schema.json
----
...
memory_config = MemoryConfig(
    default_conversation_limit=50,                 # Default messages per conversation
    message_embedding_enabled=True,                # Embed messages for similarity search
    preference_confidence_threshold=0.7,           # Confidence floor for preferences
    fact_deduplication_enabled=True,               # Deduplicate facts on ingest
    trace_embedding_enabled=True,                  # Embed reasoning traces for retrieval
    tool_stats_enabled=True,                       # Aggregate tool-call statistics
)
----
...
[source,bash]
----
NAM_MEMORY__DEFAULT_CONVERSATION_LIMIT=50
NAM_MEMORY__MESSAGE_EMBEDDING_ENABLED=true
NAM_MEMORY__PREFERENCE_CONFIDENCE_THRESHOLD=0.7
NAM_MEMORY__FACT_DEDUPLICATION_ENABLED=true
NAM_MEMORY__TRACE_EMBEDDING_ENABLED=true
NAM_MEMORY__TOOL_STATS_ENABLED=true
----
...
search_config = SearchConfig(
    ...
    default_threshold=0.7,                # Minimum similarity score
    hybrid_search_enabled=True,           # Combine vector + graph signals
    graph_depth=2,                        # Hops to traverse during graph search
)
----
...
NAM_SEARCH__DEFAULT_THRESHOLD=0.7
NAM_SEARCH__HYBRID_SEARCH_ENABLED=true
NAM_SEARCH__GRAPH_DEPTH=2
----
...
# Per-entity deduplication is configured via DeduplicationConfig
# at the memory layer (see how-to/deduplication.adoc).
# Observability tracers are obtained from
# ``neo4j_agent_memory.observability.get_tracer(...)``.
----
