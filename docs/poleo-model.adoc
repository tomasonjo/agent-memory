...
Entities are stored as nodes in Neo4j with the following structure:
...
(:Entity {type: "PERSON"})-[:LIVES_IN]->(:Entity {type: "LOCATION"})

// Organization located at location
(:Entity {type: "ORGANIZATION"})-[:LOCATED_AT]->(:Entity {type: "LOCATION"})

// Person owns object
(:Entity {type: "PERSON"})-[:OWNS]->(:Entity {type: "OBJECT"})

// Person participated in event
(:Entity {type: "PERSON"})-[:PARTICIPATED_IN]->(:Entity {type: "EVENT"})
----
...
async with MemoryClient(settings) as memory:
    ...
    vehicles = await memory.semantic.search_entities(
        query="cars",
        entity_type="OBJECT"
    )
----
...
* xref:memory-types.adoc[Work with entities in semantic memory]
