...
const NODE_COLORS: Record<string, string> = {
  Thread: "#4C8EDA", // Blue - conversation threads
  Conversation: "#4C8EDA", // Blue - conversation threads (alias)
  Message: "#57C7E3", // Cyan - individual messages
  ReasoningStep: "#8DCC93", // Green - reasoning process
  ReasoningTrace: "#8DCC93", // Green - reasoning process (alias)
  ToolCall: "#F79767", // Orange - tool executions
  Tool: "#C990C0", // Purple - canonical tools
  UserPreference: "#FFC454", // Yellow/Gold - user preferences
  Preference: "#FFC454", // Yellow/Gold - user preferences (alias)
  PreferenceCategory: "#DA7194", // Pink - preference categories
  Category: "#DA7194", // Pink - preference categories (alias)
  ...
  Entity: "#9B59B6", // Purple - generic entities
};
...
export default function MemoryGraphView({
  ...
  const loadGraphData = async () => {
    ...
    try {
      ...
      if (data.nodes.length === 0) {
        setError(
          "No memory data available. Enable memory and chat to build your memory graph.",
        );
      }
    } catch (err) {
      const errorMsg =
  ...
  const getNodeCaption = (node: GraphNode): string => {
    ...
    switch (nodeType) {
      ...
      case "Conversation": {
        ...
        if (sessionId) {
          return `Session: ${sessionId.substring(0, 20)}${sessionId.length > 20 ? "..." : ""}`;
        }
        return "Conversation";
      }
      case "Message": {
  ...
  // Handle node and relationship click events
