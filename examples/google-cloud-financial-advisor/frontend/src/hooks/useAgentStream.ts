...
export function useAgentStream() {
  ...
  const updateAgent = useCallback((agentName: string, updater: (state: AgentState) => AgentState) => {
  ...
  const startStream = useCallback(async (
    ...
    try {
      await streamChatMessage(
        { message, session_id: sessionId, customer_id: customerId, investigation_id: investigationId },
        (event: AgentEvent) => {
