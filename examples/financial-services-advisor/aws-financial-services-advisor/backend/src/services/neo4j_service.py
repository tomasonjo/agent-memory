...
class Neo4jDomainService:
    ...
    async def list_customers(self, *, customer_type: str | None = None, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        where = ""
    ...
    async def get_customer_documents(self, customer_id: str, document_type: str | None = None) -> list[dict[str, Any]]:
        where = "WHERE c.id = $id"
    ...
    async def get_transactions(self, customer_id: str, *, days: int = 90, min_amount: float | None = None, transaction_type: str | None = None) -> list[dict[str, Any]]:
    ...
    async def get_transaction_stats(self, customer_id: str) -> dict[str, Any]:
        ...
        results = await self._graph.execute_read(query, {"id": customer_id})
        if not results:
            return {"transaction_count": 0, "total_volume": 0, "total_deposits": 0, "total_withdrawals": 0, "average_transaction": 0, "counterparties": [], "transaction_types": []}
        return results[0]
    ...
    async def detect_rapid_movement(self, customer_id: str) -> list[dict[str, Any]]:
        ...
        RETURN t_in {.*} AS inbound, t_out {.*} AS outbound, t_in.amount - t_out.amount AS retained
        ORDER BY t_in.date
        """
        results = await self._graph.execute_read(query, {"id": customer_id})
        return results
    ...
    async def trace_ownership(self, entity_id: str) -> dict[str, Any]:
        ...
        RETURN owner {.id, .name, .type, .jurisdiction} AS owner, rel_types, chain_length
        ...
        return {
            ...
            "ubo_identified": any(r["owner"].get("type") in ("individual", "PERSON") for r in owners),
        }
    ...
    async def get_network_risk(self, entity_id: str) -> dict[str, Any]:
        ...
        WHERE connected <> start
        RETURN DISTINCT connected {.id, .name, .type, .jurisdiction, .shell_indicators, .role} AS entity
        """
        results = await self._graph.execute_read(query, {"id": entity_id})
        risk_score = 0
        risk_factors = []
        for r in results:
            ...
            if jurisdiction in high_risk_jurisdictions:
                risk_score += 15
                risk_factors.append(f"HIGH_RISK_JURISDICTION: {entity.get('name')} ({jurisdiction})")
            if len(indicators) > 0:
                risk_score += 20
                risk_factors.append(f"SHELL_COMPANY: {entity.get('name')} ({', '.join(indicators)})")
        ...
        return {"entity_id": entity_id, "network_risk_score": risk_score, "risk_level": risk_level, "risk_factors": risk_factors, "total_connections": len(results)}

    # ── Alerts ─────────────────────────────────────────────────────────

    async def list_alerts(self, *, status: str | None = None, severity: str | None = None, alert_type: str | None = None, customer_id: str | None = None, limit: int = 100, offset: int = 0) -> list[dict[str, Any]]:
        filters = []
        params: dict[str, Any] = {"limit": limit, "offset": offset}
        if status:
        ...
        ORDER BY CASE a.severity WHEN 'CRITICAL' THEN 0 WHEN 'HIGH' THEN 1 WHEN 'MEDIUM' THEN 2 WHEN 'LOW' THEN 3 END, a.created_at DESC
        SKIP $offset LIMIT $limit
        """
        results = await self._graph.execute_read(query, params)
    ...
    async def create_alert(self, alert: dict[str, Any]) -> dict[str, Any]:
        ...
        results = await self._graph.execute_write(query, {
            "customer_id": alert["customer_id"], "id": alert_id,
            "type": alert.get("type", "AML"), "severity": alert.get("severity", "MEDIUM"),
            "status": alert.get("status", "NEW"), "title": alert["title"],
            "description": alert.get("description", ""), "evidence": alert.get("evidence", []),
            "requires_sar": alert.get("requires_sar", False), "auto_generated": alert.get("auto_generated", False),
        })
    ...
    async def get_alert_summary(self) -> dict[str, Any]:
        ...
        if not results:
            return {"total": 0, "by_severity": {}, "by_status": {}, "critical_unresolved": 0, "high_unresolved": 0}
        r = results[0]
        return {
            "total": r["total"],
            "by_severity": {"CRITICAL": r["critical"], "HIGH": r["high"], "MEDIUM": r["medium"], "LOW": r["low"]},
            "by_status": {"NEW": r["new_count"], "ACKNOWLEDGED": r["acknowledged"], "INVESTIGATING": r["investigating"], "ESCALATED": r["escalated"], "RESOLVED": r["resolved"]},
    ...
    async def check_sanctions(self, entity_name: str, *, include_aliases: bool = True) -> list[dict[str, Any]]:
        name_lower = entity_name.lower()
        query = """
        MATCH (s:SanctionedEntity)
        OPTIONAL MATCH (alias:SanctionAlias)-[:ALIAS_OF]->(s)
        WITH s, collect(alias.name) AS aliases
        WHERE toLower(s.name) CONTAINS $name OR any(a IN aliases WHERE toLower(a) CONTAINS $name)
        ...
        """
        return await self._graph.execute_read(query, {"name": name_lower})

    # ── PEP ────────────────────────────────────────────────────────────

    async def check_pep(self, person_name: str, *, include_relatives: bool = True) -> list[dict[str, Any]]:
        ...
        RETURN p {.*} AS pep,
               CASE WHEN toLower(p.name) = $name THEN 'DIRECT_PEP' ELSE 'POTENTIAL_PEP' END AS match_type,
    ...
    async def get_graph_stats(self) -> dict[str, Any]:
        node_query = "MATCH (n) RETURN labels(n)[0] AS label, count(*) AS count ORDER BY label"
        rel_query = "MATCH ()-[r]->() RETURN type(r) AS type, count(*) AS count ORDER BY type"
